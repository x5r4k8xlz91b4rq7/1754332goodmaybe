...
import { X, ShoppingBag, Plus, Minus, Trash2, Check } from 'lucide-react';
...
export default function CartDrawer() {
  ...
  return (
    <>
      {/* Add to bag toast */}
      {showToast && (
        <div className="fixed top-24 right-6 z-[80] flex items-center gap-3 px-4 py-3 rounded-xl bg-navy text-white text-sm font-semibold premium-shadow animate-in slide-in-from-right duration-300">
      ...
      {!isOpen ? null : (
        <>
          <div
            className="fixed inset-0 bg-black/40 z-[60] drawer-backdrop"
            onClick={closeCart}
          />
          <div className="fixed right-0 top-0 bottom-0 w-full max-w-md bg-white z-[70] drawer flex flex-col">
            ...
            {items.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center gap-4 p-8 text-center">
                ...
                <button onClick={closeCart} className="btn-luxe h-11 px-6 rounded-xl text-white text-sm font-semibold">
              ...
              <>
                  ...
                  {items.map((item) => (
                    <div key={`${item.slug}-${item.size}`} className="drawer-item flex gap-4 pb-4 border-b border-[#eef1f6] last:border-0">
                      <div className="w-20 h-20 rounded-lg overflow-hidden shrink-0">
                        <ProductImage image={item.image} alt={item.name} variant="trading-card-box" className="w-full h-full" />
                      </div>
                ...
                <div className="p-5 border-t border-[#e7eaf0] space-y-4">
                  ...
                  <div className="flex gap-3">
                    ...
                    <button
                      className="btn-luxe flex-1 h-12 rounded-xl text-white text-sm font-semibold"
                      disabled
                    >
                      <span className="btn-shine" />
